...
if not os.path.exists(IMAGE_PATH + "/comp/mosaic"):
    os.makedirs(IMAGE_PATH + "/comp/mosaic")
if not os.path.exists(IMAGE_PATH + "/error"):
    os.makedirs(IMAGE_PATH + "/error")


def image_mosaic(name):
    img = Image.open(IMAGE_PATH + "/comp/" + name)
    ...
    img.save(IMAGE_PATH + "/comp/mosaic/" + name, quality=50)


def image_compression(name, san):
    imgsize = os.path.getsize(IMAGE_PATH + "/" + name)
    try:
        img = Image.open(IMAGE_PATH + "/" + name).convert("RGB")
    except Exception:
        return [1]
    img_x, img_y = img.size
    logger.info("原图分辨率为 " + str(img_x) + "x" + str(img_y))
    ...
    return [
        0,
        str(img_x) + "x" + str(img_y),
        str(int(finlsize / 1000)) + "kb / " + str(int(imgsize / 1000)) + "kb",
    ]
